# QA 专项测试计划 — 九洲 RAG 管理平台

> **版本**: 1.1  
> **日期**: 2025-03-01  
> **测试范围**: 知识库问答、知识库构建、系统管理 Dashboard、CLI 脚本、Provider 切换、数据生命周期、容错降级  
> **测试环境**: Windows, Python 3.11+, 虚拟环境 `.venv`  
> **当前配置**: DashScope (LLM: qwen-plus / Embedding: text-embedding-v3 / Vision: qwen-vl-plus)

---

## 目录

- [QA. Dashboard — 知识库问答页面（新增）](#qa-dashboard--知识库问答页面)
- [KB. Dashboard — 知识库构建页面（新增）](#kb-dashboard--知识库构建页面)
...
> 启动方式: `python scripts/start_dashboard.py` → 浏览器打开 `http://localhost:8501`，默认即为知识库问答页

| ID | 测试标题 | 状态 | 操作步骤 | 预期现象 |
|----|---------|------|---------|--------- |
| QA-01 | 知识库问答页面默认加载 | Baseline | 1. 打开浏览器访问 `http://localhost:8501` | 页面标题显示“💬 知识库问答”，侧边栏显示问答设置，底部显示聊天输入框，无报错 |
| QA-02 | 侧边栏导航结构 | Baseline | 1. 查看侧边栏导航 | “💬 知识库问答”始终可见；“系统管理”分组默认折叠，展开后显示知识库构建、系统总览等 6 个子页面 |
| QA-03 | 检索模式切换 — 每次都检索 | Baseline | 1. 在对话框上方选择“📚 每次都检索”<br>2. 输入问题“什么是混合检索？” | 显示“正在检索知识库并生成回答…”，生成包含检索结果引用的回答 |
| QA-04 | 检索模式切换 — 不检索 | Baseline | 1. 选择“🚫 不检索”<br>2. 输入问题“什么是混合检索？” | 显示“正在生成回答…”（无“检索知识库”字样），回答基于模型自身知识，无参考资料区域 |
| QA-05 | 检索模式切换 — 自动判断 | Baseline | 1. 选择“🤖 模型自动判断”<br>2. 输入问题“你好” | 显示“🤖 正在判断是否需要检索知识库…”，对于闲聊问题应判断为“不需要检索”，显示“💡 模型判断本次无需检索知识库” |
| QA-06 | 自动判断 — 专业问题触发检索 | Baseline | 1. 选择“🤖 模型自动判断”<br>2. 输入专业问题“混合检索中 RRF 融合算法的具体实现方式是什么？” | 模型判断为“需要检索”，正常检索知识库并生成回答 |
| QA-07 | 文件上传 — 单文件 | Baseline | 1. 点击“📎 上传文档”上传一个 PDF 文件<br>2. 输入问题“请根据上传的文档写一份技术方案” | 显示“已加载 1 个文档”，回答中包含上传文档的内容引用，用户消息显示“📎 附件：xxx.pdf” |
| QA-08 | 文件上传 — 多文件 | Baseline | 1. 上传多个文件（PDF + DOCX）<br>2. 输入问题 | 显示“已加载 N 个文档”，回答综合多个文档内容 |
| QA-09 | 文件上传 + 不检索模式 | Baseline | 1. 上传文档<br>2. 选择“🚫 不检索”<br>3. 输入问题 | 仅基于上传文档生成回答，不检索知识库 |
| QA-10 | 超长文档自动续写 | Baseline | 1. 输入需要长回答的问题，如“请写一份完整的技术方案，包括项目背景、技术架构、实施计划” | 生成完整的长文档（可能经过多轮续写），内容不中断，正确衔接 |
| QA-11 | Mermaid 图表渲染 | Baseline | 1. 输入“请画一个 RAG 系统架构图” | 回答中包含 Mermaid 代码块，页面渲染为可视化图表 |
| QA-12 | 数据图表渲染 | Baseline | 1. 输入涉及数据对比的问题 | 回答中生成图表代码块，渲染为柱状图/饼图/折线图 |
| QA-13 | 知识库图片显示 | Baseline | 1. 侧边栏勾选“显示知识库图片”<br>2. 查询含图片的文档内容 | 回答下方显示从检索结果提取的相关图片 |
| QA-14 | 方案导出为 Markdown | Baseline | 1. 生成一个回答后<br>2. 点击“导出”按钮 | 下载 .md 文件，内容与页面显示一致 |
| QA-15 | 空知识库查询（无上传文档） | Empty | 1. 选择“📚 每次都检索”<br>2. 输入问题 | 显示“⚠️ 未在知识库中找到相关内容”警告信息，不崩溃 |
| QA-16 | 对话历史保持 | Baseline | 1. 连续提问多个问题 | 所有历史问答在页面上保留显示 |
...
| ID | 测试标题 | 状态 | 操作步骤 | 预期现象 |
|----|---------|------|---------|--------- |
| KB-01 | 知识库构建页面加载 | Any | 1. 侧边栏展开“系统管理”<br>2. 点击“知识库构建” | 页面显示文件夹路径输入、集合名称输入、开始构建按钮 |
| KB-02 | 批量摄取正常流程 | Baseline | 1. 输入含文档的文件夹路径<br>2. 点击“开始构建知识库” | 进度条实时更新，显示当前处理文件和阶段，最终显示完成结果 |
| KB-03 | 摄取进度页面切换不丢失 | Baseline | 1. 开始摄取<br>2. 切换到其他页面<br>3. 切回知识库构建页 | 进度和结果仍然显示，未丢失 |
| KB-04 | 摄取过程中禁用输入 | Baseline | 1. 开始摄取后观察输入控件 | 文件夹路径、集合名称输入框和开始按钮均被禁用，防止重复触发 |
| KB-05 | 空文件夹路径 | Any | 1. 输入不存在的路径<br>2. 点击开始构建 | 显示路径无效或无文档的错误提示，不崩溃 |
| KB-06 | 增量摄取（重复文件跳过） | Baseline | 1. 对已摄取过的文件夹再次执行构建 | 已处理文件被跳过，进度快速完成 |
