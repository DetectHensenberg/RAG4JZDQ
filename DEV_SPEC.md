...
本项目（九洲 RAG 管理平台）基于多阶段检索增强生成（RAG, Retrieval-Augmented Generation）与模型上下文协议（MCP, Model Context Protocol）设计，目标是搭建一个可扩展、高可观测、易迭代的智能问答与知识检索框架，支撑方案人员的日常工作。
...
- **可视化管理平台 (Visual Management Dashboard)**：
  - 基于 Streamlit 的本地 Web 管理面板，提供两大主功能 + 六项系统管理功能：
    - **💬 知识库问答**（默认页）：基于知识库检索 + LLM 生成专业方案，支持文件上传（招标文件/需求文档/目录模板）、三种检索模式切换、Mermaid 图表、数据图表、超长文档自动续写、方案导出。
    - **📂 知识库构建**：指定本地文件夹一键构建知识库，后台线程执行，页面切换后进度不丢失。
    - **系统管理**（侧边栏折叠）：系统总览、数据浏览器、Ingestion 管理、Ingestion 追踪、Query 追踪、评估面板。
...
### 业务可扩展性 (Extensibility)

本项目采用**通用化架构设计**，不仅是一个开箱即用的知识问答系统，更是一个可以快速适配各类业务场景的**扩展基座**：

- **Agent 客户端扩展 (Agent Client Extension)**：

  - 本项目的 MCP Server 天然支持被各类 Agent 调用，可基于此构建定制化的 Agent 客户端：
...
- **业务场景快速适配 (Adapt to Your Domain)**：
  ...
  - **数据层扩展**：只需替换数据源（接入文档、数据库、API），即可将本系统改造为私有知识库；
  - **检索逻辑定制**：基于可插拔架构，轻松调整检索策略以适配不同业务特点（如电商搜索偏重关键词、法律文档偏重语义）；
  - **Prompt 模板定制**：修改系统 Prompt 和输出格式，使其符合业务风格与专业术语。
...
-  **Pipeline 框架（设计灵感参考 LlamaIndex 分层思想，但不依赖 LlamaIndex 库）：**
...
- **备选：FastAPI + 自定义协议层**
  ...
  - **场景**：需要深度定制 HTTP 行为（如自定义中间件、复杂鉴权流程）或需要完全掌控 MCP 协议底层细节时可考虑。
...
- **维护成本低**：结构化日志是通用技能，调试时可直接用 `jq`、`grep` 等命令行工具查询；Dashboard 代码简单直观，便于二次开发。
...
Dashboard 基于 Streamlit 构建多页面应用（`st.navigation`），提供两大主功能 + 六项系统管理功能。侧边栏导航中，知识库问答始终可见，系统管理页面默认折叠（`expanded=False`）。

**页面 0：知识库问答 (Knowledge QA) — 默认页**

- **核心功能**：基于知识库检索 + LLM 生成专业方案文档
- **文件上传**：支持 PDF、DOCX、PPTX、XLSX、TXT、MD、CSV 等格式，作为招标文件、需求文档或目录模板输入给 LLM，使用 MarkItDown 解析
- **检索模式**：三种模式横向切换（🤖 模型自动判断 / 📚 每次都检索 / 🚫 不检索），自动判断模式通过轻量 LLM 调用决定是否需要检索
- **超长文档续写**：检测 `finish_reason='length'` 自动分段续写（最多 6 轮），生成完整长文档
- **富内容渲染**：自动生成 Mermaid 架构图/流程图、数据图表（柱状图/饼图/折线图）
- **知识库图片**：从检索结果中提取并展示相关图片
- **AI 配图**：可选通过通义万相 API 生成方案配图
- **方案导出**：支持导出为 Markdown 文档

**页面 0.5：知识库构建 (Knowledge Base Builder)**

- **目录扫描**：指定本地文件夹，自动递归扫描 PDF/DOCX/TXT/MD 文档
- **批量摄取**：一键触发批量 Ingestion Pipeline，后台线程执行
- **进度持久化**：进度和结果存储在 `st.session_state`，页面切换后不丢失
- **增量摄取**：基于文件哈希自动跳过已处理文件
...
├── app.py                    # Streamlit 入口，页面导航注册（dict 分组 + expanded=False）
├── pages/
│   ├── knowledge_qa.py       # 页面 0：知识库问答（默认页，支持文件上传/检索模式/富内容/续写）
│   ├── knowledge_base.py     # 页面 0.5：知识库构建（后台线程 + session_state 持久化）
...
- **测试即文档**：测试用例本身就是最准确的行为规范，可通过阅读测试快速理解各模块功能。
- **快速反馈循环**：单元测试应在秒级完成，支持高频执行，立即发现引入的问题。
...
- **M6（完成阶段 H+I）**：评估体系完整 + E2E 验收通过 + 文档完善，形成可复现的完整工程项目。

## 7. 可扩展性与未来展望

### 7.1 云端部署与后端架构演进

虽然当前阶段主要采用"本地运行"模式，但本项目的架构设计完全支持向云端迁移。

- **Server 容器化**：计划编写 Dockerfile，将 MCP Server 打包为容器，实现标准化的环境隔离与依赖管理。
- **云端接入**：未来可以将 Server 部署至 Azure Container Apps 或 AWS Lambda。
  - **关键挑战**：处理网络延时、配置 API Gateway、增加 AuthN/AuthZ 鉴权机制（保护私有数据不被公开访问）。
- **多租户与并发**：从单用户本地服务转变为支持团队共享的服务。
  - **关键挑战**：在 Chroma 中实现 Namespace 隔离、处理并发请求锁、优化 embedding 缓存策略。
